...
def get_connection():
    return pymysql.connect(host='localhost', user='root', passwd='Sunmoon2730', database='redbus')
...
def fetch_route_names(connection, starting_letter):
    query = f"SELECT DISTINCT Bus_Name FROM red_bus_details WHERE Bus_Name LIKE '{starting_letter}%' ORDER BY Bus_Name"
    route_names = pd.read_sql(query, connection)['Bus_Name'].tolist()
    return route_names
...
def fetch_data(connection, route_name, price_sort_order):
    ...
    query = f"SELECT * FROM red_bus_details WHERE Bus_Name = %s ORDER BY Star_Rating DESC, Price {price_sort_order_sql}"
